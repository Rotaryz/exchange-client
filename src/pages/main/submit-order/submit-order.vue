        ...
        <li v-for="(item,i) in order.list" :key="i" class="goods-item">
          <div class="item-wrap">
            ...
            <div class="info">
              ...
              <div class="num-wrap">
                <div class="pay-money mini-small common">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      // 去地址页面选择地址
      goMyAddress() {
        wx.navigateTo({ url: this.$routes.personal.MY_ADDRESS + '?save=1' })
      },
      // 确认支付下单
      submitBtn() {
        ...
        API.Order.placeOrder({ data, loading: false }).then(res => {
          this._payFor(res.data.pay_config).then(() => {
            ...
            wx.redirectTo({ url: this.$routes.main.PAY_SUCCESS + '?id=' + res.data.id })
          }).catch(() => {
            wx.redirectTo({ url: this.$routes.personal.ORDER_DETAIL + '?id=' + res.data.id })
          })
        })
      }
    }
